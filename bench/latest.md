...
**Version** 0.1.2 · **Bun** 1.4.0 · **Date** 2026-09-05T06:39:08.595Z · **Tokenizer** o200k_base

| Metric | Value | Budget | Status | Note |
|---|---|---|---|---|
| `build.cold.100` | 305.6 ms | 2000 ms | ✅ | 113 routes, no dist, no index |
| `build.cold.1000` | 2722.4 ms | 20000 ms | ✅ | 1013 routes, no dist, no index |
| `build.cold.10000` | 27481.6 ms | 200000 ms | ✅ | 10013 routes, no dist, no index |
| `build.incremental.100` | 9.8 ms | 300 ms | ✅ | one body edit → 1 rendered, 121 cached |
| `build.noop.100` | 6.4 ms | — | report | touch only (mtime): stat + one hash, nothing rendered; report-only |
| `lint.100` | 11 ms | 100 ms | ✅ | 0 errors · 0 warnings; mdast cache warm |
| `lint.100.cold` | 213.8 ms | — | report | parse (micromark) + lint from an empty cache; report-only |
| `lint.1000` | 67.7 ms | 1000 ms | ✅ | 0 errors · 0 warnings; mdast cache warm |
| `lint.1000.cold` | 2180.9 ms | — | report | parse (micromark) + lint from an empty cache; report-only |
| `mcp.coldStart.binary` | 22.3 ms | 50 ms | ✅ | the artefact a release ships (`bun build --compile --splitting`), spawn → `initialize`; D2's lane since S18c · median of 21 interleaved rounds |
...
| `desk.ttfb` | 2.4 ms | — | report | report-only (S19a): `/_snypd` on the same server and the same unchanged tree. The Desk inherits D2's 50 ms rather than owning it, and this is the first session that can say whether it is inside it |
...
| `tokens.tools.full` | 3461 tokens | — | report | the same 14 tools with the catalogue listed rather than found — what deferring it saves a turn (docs/07 decision 38); report-only |
...
| `viz.chart.renderMs` | 0.11 ms | 3 ms | ✅ | worst type (bar) on the worst shape — bar 0.11 ms / 6.5 KB · line 0.07 ms / 4.3 KB · area 0.07 ms / 4.5 KB · donut 0.10 ms / 6.9 KB · lollipop 0.09 ms / 7.6 KB |
| `viz.chart.svgKb` | 7.6 KB | 12 KB | ✅ | worst type (lollipop); zero JS, zero CSS |
| `viz.diagram.renderMs` | 0.97 ms | 15 ms | ✅ | worst shape (feedback) at the 40-node cap, layout cache defeated — chain 0.66 ms / 10.5 KB · wide 0.82 ms / 11.9 KB · feedback 0.97 ms / 13.2 KB |
| `viz.diagram.svgKb` | 13.2 KB | 25 KB | ✅ | worst shape (feedback); zero JS, zero CSS |
| `viz.flow.renderMs` | 0.46 ms | 15 ms | ✅ | worst shape (nested) at the 40-node cap, layout cache defeated — ladder 40 steps 0.44 ms / 13.7 KB · retry loop 38 steps 0.43 ms / 13.2 KB · nested 40 steps 0.46 ms / 14.4 KB |
...
| `suggest.ms` | 4.02 ms | — | report | per post: parse + shapes + score + verify (the verify pass lints each candidate against the document it would land in) |
...
| `page.lcp` | 64 ms | — | report | worst /posts/every-primitive-once/ @ 390; localhost, unthrottled — the shape of the page, not a field number; report-only |
...
| `desk.bytes.kb` | 17.93 KB | — | report | worst /_snypd @ 1280: 8.11 KB html + 9.83 KB css + 0 KB img, 2 requests — uncompressed, which no host serves; report-only |
| `desk.lcp` | 64 ms | — | report | worst /_snypd @ 1280; localhost, unthrottled — the shape of the page, not a field number; report-only |
...
| `desk.first.bytes.kb` | 20.17 KB | — | report | worst /_snypd @ 1280: 10.34 KB html + 9.83 KB css + 0 KB img, 2 requests — uncompressed, which no host serves; report-only |
