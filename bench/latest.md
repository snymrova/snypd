...
**Version** 0.1.0-s18c · **Bun** 1.4.0 · **Date** 2026-08-28T17:37:26.751Z · **Tokenizer** o200k_base

| Metric | Value | Budget | Status | Note |
|---|---|---|---|---|
| `build.cold.100` | 717.9 ms | 2000 ms | ✅ | 113 routes, no dist, no index |
| `build.cold.1000` | 5870.4 ms | 20000 ms | ✅ | 1013 routes, no dist, no index |
| `build.cold.10000` | 59174.1 ms | 200000 ms | ✅ | 10013 routes, no dist, no index |
| `build.incremental.100` | 22.4 ms | 300 ms | ✅ | one body edit → 1 rendered, 121 cached |
| `build.noop.100` | 13.5 ms | — | report | touch only (mtime): stat + one hash, nothing rendered; report-only |
| `lint.100` | 17.1 ms | 100 ms | ✅ | 0 errors · 0 warnings; mdast cache warm |
| `lint.100.cold` | 459.9 ms | — | report | parse (micromark) + lint from an empty cache; report-only |
| `lint.1000` | 120.6 ms | 1000 ms | ✅ | 0 errors · 0 warnings; mdast cache warm |
| `lint.1000.cold` | 4703 ms | — | report | parse (micromark) + lint from an empty cache; report-only |
| `mcp.coldStart.binary` | 38.6 ms | 50 ms | ✅ | the artefact a release ships (`bun build --compile --splitting`), spawn → `initialize`; D2's lane since S18c · median of 21 interleaved rounds |
| `mcp.coldStart` | 37.1 ms | — | report | report-only since S18c: `bun packages/mcp/src/server.ts`, the dev loop, not the thing anyone installs — interleaved with the binary lane, so the delta between the two rows is real even when the box is loaded |
| `serve.ttfb` | 0.16 ms | — | report | static dist/ over Bun.serve — the floor the preview server is measured against |
| `preview.ttfb` | 0.17 ms | 50 ms | ✅ | serve --preview, unchanged tree, drafts included; review page served |
...
| `tokens.tools.full` | 3291 tokens | — | report | the same 14 tools with the catalogue listed rather than found — what deferring it saves a turn (docs/07 decision 38); report-only |
...
| `viz.chart.renderMs` | 0.26 ms | 3 ms | ✅ | worst type (donut) on the worst shape — bar 0.22 ms / 6.5 KB · line 0.13 ms / 4.3 KB · area 0.13 ms / 4.5 KB · donut 0.26 ms / 6.9 KB · lollipop 0.22 ms / 7.6 KB |
| `viz.chart.svgKb` | 7.6 KB | 12 KB | ✅ | worst type (lollipop); zero JS, zero CSS |
| `viz.diagram.renderMs` | 1.74 ms | 15 ms | ✅ | worst shape (feedback) at the 40-node cap, layout cache defeated — chain 1.23 ms / 10.5 KB · wide 1.33 ms / 11.9 KB · feedback 1.74 ms / 13.2 KB |
| `viz.diagram.svgKb` | 13.2 KB | 25 KB | ✅ | worst shape (feedback); zero JS, zero CSS |
| `viz.flow.renderMs` | 0.8 ms | 15 ms | ✅ | worst shape (nested) at the 40-node cap, layout cache defeated — ladder 40 steps 0.75 ms / 13.7 KB · retry loop 38 steps 0.80 ms / 13.2 KB · nested 40 steps 0.80 ms / 14.4 KB |
...
| `suggest.ms` | 7.96 ms | — | report | per post: parse + shapes + score + verify (the verify pass lints each candidate against the document it would land in) |
...
| `page.lcp` | 128 ms | — | report | worst /posts/every-primitive-once/ @ 1280; localhost, unthrottled — the shape of the page, not a field number; report-only |
...
| `desk.bytes.kb` | 13.68 KB | — | report | worst /_snypd @ 1280: 3.85 KB html + 9.83 KB css + 0 KB img, 2 requests — uncompressed, which no host serves; report-only |
| `desk.lcp` | 116 ms | — | report | worst /_snypd/review/post/a-draft-in-flight @ 1280; localhost, unthrottled — the shape of the page, not a field number; report-only |
