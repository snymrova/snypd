...
**Version** 0.1.0-s18c · **Bun** 1.4.0 · **Date** 2026-08-31T18:11:15.058Z · **Tokenizer** o200k_base

| Metric | Value | Budget | Status | Note |
|---|---|---|---|---|
| `build.cold.100` | 242.4 ms | 2000 ms | ✅ | 113 routes, no dist, no index |
| `build.cold.1000` | 2295.7 ms | 20000 ms | ✅ | 1013 routes, no dist, no index |
| `build.cold.10000` | 23372.3 ms | 200000 ms | ✅ | 10013 routes, no dist, no index |
| `build.incremental.100` | 7.5 ms | 300 ms | ✅ | one body edit → 1 rendered, 121 cached |
| `build.noop.100` | 4.8 ms | — | report | touch only (mtime): stat + one hash, nothing rendered; report-only |
| `lint.100` | 7.6 ms | 100 ms | ✅ | 0 errors · 0 warnings; mdast cache warm |
| `lint.100.cold` | 175.4 ms | — | report | parse (micromark) + lint from an empty cache; report-only |
| `lint.1000` | 57.7 ms | 1000 ms | ✅ | 0 errors · 0 warnings; mdast cache warm |
| `lint.1000.cold` | 1759.3 ms | — | report | parse (micromark) + lint from an empty cache; report-only |
| `mcp.coldStart.binary` | 18.4 ms | 50 ms | ✅ | the artefact a release ships (`bun build --compile --splitting`), spawn → `initialize`; D2's lane since S18c · median of 21 interleaved rounds |
| `mcp.coldStart` | 17.7 ms | — | report | report-only since S18c: `bun packages/mcp/src/server.ts`, the dev loop, not the thing anyone installs — interleaved with the binary lane, so the delta between the two rows is real even when the box is loaded |
| `serve.ttfb` | 0.09 ms | — | report | static dist/ over Bun.serve — the floor the preview server is measured against |
| `preview.ttfb` | 0.1 ms | 50 ms | ✅ | the preview server (`snypd dev`), unchanged tree, drafts included; review page served |
...
| `viz.chart.renderMs` | 0.12 ms | 3 ms | ✅ | worst type (bar) on the worst shape — bar 0.12 ms / 6.5 KB · line 0.05 ms / 4.3 KB · area 0.08 ms / 4.5 KB · donut 0.08 ms / 6.9 KB · lollipop 0.09 ms / 7.6 KB |
| `viz.chart.svgKb` | 7.6 KB | 12 KB | ✅ | worst type (lollipop); zero JS, zero CSS |
| `viz.diagram.renderMs` | 0.82 ms | 15 ms | ✅ | worst shape (feedback) at the 40-node cap, layout cache defeated — chain 0.73 ms / 10.5 KB · wide 0.68 ms / 11.9 KB · feedback 0.82 ms / 13.2 KB |
| `viz.diagram.svgKb` | 13.2 KB | 25 KB | ✅ | worst shape (feedback); zero JS, zero CSS |
| `viz.flow.renderMs` | 0.62 ms | 15 ms | ✅ | worst shape (retry loop) at the 40-node cap, layout cache defeated — ladder 40 steps 0.60 ms / 13.7 KB · retry loop 38 steps 0.62 ms / 13.2 KB · nested 40 steps 0.34 ms / 14.4 KB |
...
| `suggest.ms` | 3.38 ms | — | report | per post: parse + shapes + score + verify (the verify pass lints each candidate against the document it would land in) |
...
| `page.lcp` | 56 ms | — | report | worst /posts/every-primitive-once/ @ 390; localhost, unthrottled — the shape of the page, not a field number; report-only |
...
| `desk.lcp` | 80 ms | — | report | worst /_snypd @ 390; localhost, unthrottled — the shape of the page, not a field number; report-only |
...
| `desk.first.lcp` | 56 ms | — | report | worst /_snypd @ 1280; localhost, unthrottled — the shape of the page, not a field number; report-only |
