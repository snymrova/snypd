...
**Version** 0.1.0-s16 · **Bun** 1.4.0 · **Date** 2026-08-28T09:06:27.159Z · **Tokenizer** o200k_base

| Metric | Value | Budget | Status | Note |
|---|---|---|---|---|
| `build.cold.100` | 776 ms | 2000 ms | ✅ | 113 routes, no dist, no index |
| `build.cold.1000` | 8469.5 ms | 20000 ms | ✅ | 1013 routes, no dist, no index |
| `build.cold.10000` | 63269.9 ms | 200000 ms | ✅ | 10013 routes, no dist, no index |
| `build.incremental.100` | 19.1 ms | 300 ms | ✅ | one body edit → 1 rendered, 121 cached |
| `build.noop.100` | 13.3 ms | — | report | touch only (mtime): stat + one hash, nothing rendered; report-only |
| `lint.100` | 14.9 ms | 100 ms | ✅ | 0 errors · 0 warnings; mdast cache warm |
| `lint.100.cold` | 518 ms | — | report | parse (micromark) + lint from an empty cache; report-only |
| `lint.1000` | 131.7 ms | 1000 ms | ✅ | 0 errors · 0 warnings; mdast cache warm |
| `lint.1000.cold` | 5415.5 ms | — | report | parse (micromark) + lint from an empty cache; report-only |
| `mcp.coldStart` | 44.2 ms | 50 ms | ⚠️ over CI (80 %) |  |
...
| `tokens.learn` | 4571 tokens | 6000 tokens | ✅ | 17 resources |
...
| `tokens.learn.editorial` | 4775 tokens | 6000 tokens | ✅ | 17 resources · editorial theme |
...
| `viz.chart.renderMs` | 0.24 ms | 3 ms | ✅ | worst type (donut) on the worst shape — bar 0.19 ms / 6.5 KB · line 0.15 ms / 4.3 KB · area 0.16 ms / 4.5 KB · donut 0.24 ms / 6.9 KB · lollipop 0.24 ms / 7.6 KB |
| `viz.chart.svgKb` | 7.6 KB | 12 KB | ✅ | worst type (lollipop); zero JS, zero CSS |
| `viz.diagram.renderMs` | 1.94 ms | 15 ms | ✅ | worst shape (feedback) at the 40-node cap, layout cache defeated — chain 1.33 ms / 10.5 KB · wide 1.53 ms / 11.9 KB · feedback 1.94 ms / 13.2 KB |
| `viz.diagram.svgKb` | 13.2 KB | 25 KB | ✅ | worst shape (feedback); zero JS, zero CSS |
| `viz.flow.renderMs` | 0.84 ms | 15 ms | ✅ | worst shape (ladder) at the 40-node cap, layout cache defeated — ladder 40 steps 0.84 ms / 13.7 KB · retry loop 38 steps 0.72 ms / 13.2 KB · nested 40 steps 0.69 ms / 14.4 KB |
...
| `suggest.ms` | 7.2 ms | — | report | per post: parse + shapes + score + verify (the verify pass lints each candidate against the document it would land in) |
...
| `page.lcp` | 196 ms | — | report | worst /category/engineering/ @ 1280; localhost, unthrottled — the shape of the page, not a field number; report-only |
