...
**Version** 0.1.0-s18c · **Bun** 1.4.0 · **Date** 2026-08-28T16:51:26.232Z · **Tokenizer** o200k_base

| Metric | Value | Budget | Status | Note |
|---|---|---|---|---|
| `build.cold.100` | 751.5 ms | 2000 ms | ✅ | 113 routes, no dist, no index |
| `build.cold.1000` | 6650.1 ms | 20000 ms | ✅ | 1013 routes, no dist, no index |
| `build.cold.10000` | 72258.1 ms | 200000 ms | ✅ | 10013 routes, no dist, no index |
| `build.incremental.100` | 21.1 ms | 300 ms | ✅ | one body edit → 1 rendered, 121 cached |
| `build.noop.100` | 12.6 ms | — | report | touch only (mtime): stat + one hash, nothing rendered; report-only |
| `lint.100` | 18.8 ms | 100 ms | ✅ | 0 errors · 0 warnings; mdast cache warm |
| `lint.100.cold` | 563 ms | — | report | parse (micromark) + lint from an empty cache; report-only |
| `lint.1000` | 113.6 ms | 1000 ms | ✅ | 0 errors · 0 warnings; mdast cache warm |
| `lint.1000.cold` | 4976.1 ms | — | report | parse (micromark) + lint from an empty cache; report-only |
| `mcp.coldStart.binary` | 39.5 ms | 50 ms | ✅ | the artefact a release ships (`bun build --compile --splitting`), spawn → `initialize`; D2's lane since S18c · median of 21 interleaved rounds |
| `mcp.coldStart` | 39.2 ms | — | report | report-only since S18c: `bun packages/mcp/src/server.ts`, the dev loop, not the thing anyone installs — interleaved with the binary lane, so the delta between the two rows is real even when the box is loaded |
...
| `viz.chart.renderMs` | 0.23 ms | 3 ms | ✅ | worst type (donut) on the worst shape — bar 0.18 ms / 6.5 KB · line 0.13 ms / 4.3 KB · area 0.14 ms / 4.5 KB · donut 0.23 ms / 6.9 KB · lollipop 0.18 ms / 7.6 KB |
| `viz.chart.svgKb` | 7.6 KB | 12 KB | ✅ | worst type (lollipop); zero JS, zero CSS |
| `viz.diagram.renderMs` | 1.64 ms | 15 ms | ✅ | worst shape (feedback) at the 40-node cap, layout cache defeated — chain 1.08 ms / 10.5 KB · wide 1.41 ms / 11.9 KB · feedback 1.64 ms / 13.2 KB |
| `viz.diagram.svgKb` | 13.2 KB | 25 KB | ✅ | worst shape (feedback); zero JS, zero CSS |
| `viz.flow.renderMs` | 0.82 ms | 15 ms | ✅ | worst shape (retry loop) at the 40-node cap, layout cache defeated — ladder 40 steps 0.73 ms / 13.7 KB · retry loop 38 steps 0.82 ms / 13.2 KB · nested 40 steps 0.76 ms / 14.4 KB |
...
| `suggest.ms` | 8.94 ms | — | report | per post: parse + shapes + score + verify (the verify pass lints each candidate against the document it would land in) |
...
| `page.lcp` | 120 ms | — | report | worst /posts/every-primitive-once/ @ 390; localhost, unthrottled — the shape of the page, not a field number; report-only |
