# snypd — the clean machine

The walk in docs/08 §2, against `node_modules/.bin/snypd` — the launcher, its platform package, and npm's link — rather than against a binary compiled from this checkout (S28, docs/31 §5 · L4).

- **box** yes — `docker/box`, nothing of this machine's on PATH but what the image installs
- **node** /usr/bin/node · v20.19.2
- **a `snypd` already on PATH** no
- **git identity** **none** — expect the walk to pay one more action

# snypd bench — onboard (released launcher)

**Version** 0.1.6 · **Bun** 1.4.0 · **Date** 2026-09-22T16:54:55.621Z · **Tokenizer** o200k_base
...
| `onboard.ttfv` | 13.93 s | 5 s | ❌ over budget | a directory that does not exist yet → `init` → `dev` → the Desk answering 200, against the artefact named at the foot of this file (docs/08 F6) |
| `onboard.ttfp` | 17.15 s | 60 s | ✅ | the typed line → a lint-clean draft with a review URL · driver: reference driver (no model) — no model latency in this number, and S21 substitutes three that have it |
| `onboard.published` | 17.55 s | — | report | …and on to a published post — report-only: it is bounded by how fast a person reads |
...
Driver `reference driver (no model)` · binary `/tmp/snypd-release-rCmHqV/node_modules/.bin/snypd` · review URL handed back: yes · lint clean: yes
...
**1** — a signal to the launcher does not reach the binary it `spawnSync`ed, so `dev` outlived the walk that started it (docs/08 §12.13). Killed here; the defect is the product's.

    68 /tmp/snypd-release-rCmHqV/npm/@snypd/linux-x64/bin/snypd dev .
