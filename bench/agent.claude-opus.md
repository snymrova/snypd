...
**Version** 0.1.4 · **Bun** 1.4.0 · **Date** 2026-09-15T09:24:16.851Z · **Tokenizer** o200k_base

| Metric | Value | Budget | Status | Note |
|---|---|---|---|---|
| `agent.goal` | 1  | ≥ 1  | ✅ | 15/15 checks, driver `claude:opus` |
| `agent.calls.draft` | 3 calls | 8 calls | ✅ | D1 literally: nothing → a lint-clean draft of the new post |
| `agent.calls` | 23 calls | — | report | upgrade 7 · theme 6 · write 3 · publish 5 · build 2 · +1 final lint — reference route 18, gated exactly at 19 in the test |
| `agent.reads` | 7 reads | — | report | resources and tools/list — free by decision 38, counted so the split stays honest |
| `agent.tokens` | 17437 tokens | — | report | o200k both directions — 1483 sent, 15954 returned |
| `agent.wallMs` | 110450 ms | — | report | spawn → published site, report-only (build and preview dominate) |
...
| `agent.model.tokens` | 551188 tokens | — | report | what `claude-opus-5`'s own context paid — 544218 in (cache included), 6970 out, 30 turns, $0.8615; ended `success`; report-only |
