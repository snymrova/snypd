...
**Version** 0.1.4 · **Bun** 1.4.0 · **Date** 2026-09-15T09:20:32.276Z · **Tokenizer** o200k_base

| Metric | Value | Budget | Status | Note |
|---|---|---|---|---|
| `agent.goal` | 1  | ≥ 1  | ✅ | 15/15 checks, driver `claude:haiku` |
| `agent.calls.draft` | 3 calls | 8 calls | ✅ | D1 literally: nothing → a lint-clean draft of the new post |
| `agent.calls` | 30 calls | — | report | upgrade 15 · theme 5 · write 3 · publish 4 · build 3 · +1 final lint — reference route 18, gated exactly at 19 in the test |
| `agent.reads` | 5 reads | — | report | resources and tools/list — free by decision 38, counted so the split stays honest |
| `agent.tokens` | 13461 tokens | — | report | o200k both directions — 1450 sent, 12011 returned |
| `agent.wallMs` | 74287 ms | — | report | spawn → published site, report-only (build and preview dominate) |
...
| `agent.model.tokens` | 521017 tokens | — | report | what `claude-haiku-4-5-20251001`'s own context paid — 514927 in (cache included), 6090 out, 35 turns, $0.2047; ended `success`; report-only |
