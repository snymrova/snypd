...
| Harness user | "how do I start" | 1 minute | the one sentence to paste, and that `bunx @snypd/cli init` is the whole install |
...
| 2 | **The hero video** (45 s) | one sentence pasted into Claude Code → `init` → a post with a chart, a flow and an FAQ → build → the site, in `editorial` | a real session, recorded (§4 B) |
| 3 | **Start here** | the paste sentence; `bunx @snypd/cli init`; the five platforms by name (docs/10 §9: every Bun platform) | `packaging/` |
...
|---|---|---|---|
| **V1 hero** | 45 s | Claude Code, the paste sentence, `init`, `write-post` with three primitives, `build`, the page in the browser | `vhs` (§4 B) driving a real interactive session; the browser half is a CDP capture stitched by `ffmpeg` |
