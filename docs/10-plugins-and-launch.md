...
| `indexnow` | 2 + 3 (+ 4 in P4) | `emit`: `dist/indexnow/<key>.txt`, the key file the protocol verifies, under its own prefix; `push` event POSTs the URLs the push changed (deletions included) to `api.indexnow.org` with `keyLocation`; `network:` is exactly the protocol's five endpoints; nothing changed → nothing sent, and it says which reason. **Shipped P3** (`plugins/indexnow`). P4 adds the `indexnow › ping` tool and `snypd://indexnow/last` | Network with an allowlist, an event, an emit that makes the event honest, and in P4 a catalogue tool and a resource, in one small package |
