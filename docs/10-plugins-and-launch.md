...
**Done in U6b** (docs/11 §7b), written after `technical` and not before it, and the answer to the question it was landed to answer is *almost*: the contract was one part and one field short of a contents list (docs/11 decisions 135, 136). Eight steps, in the order the session took them, and the last one has teeth — **look at it, at a phone width and a desktop one, in light and in dark**. A theme is the only thing in this product whose defects are invisible to every other gate: the build is green, the lint is clean, the tokens validate, and the masthead still wraps into three lines on a phone with a slash stranded at the start of one of them. `technical` shipped three fixes that came from nothing but a screenshot, and the prompt says so. Its text costs `tokens.learn` nothing: `prompts/list` carries names and descriptions, and the body is returned only to whoever asked for it.
...
- **Shelf:** `snypd.rocks/themes` and `snypd.rocks/plugins` — two pages, written through the MCP like every other page on the site, one `figure` per theme variation (screenshots rendered by a script in this repo, the bench's CDP screenshotter over the `corpora/theme` fixture at 1280 and 390) and one `steps` block per plugin showing the two lines of YAML that enable it. Hand-maintained at launch. **Done in S22 · L1, 15 Sep 2026** (docs/11 §7b): the script is `snypd bench gallery`, a lane and not only a script — six looks, each measured with the page suite's own gates before it is photographed — and `snypd://themes` is the read an agent makes to choose from the same shelf. Generated from an npm keyword search when `jobs:` lands (v0.3), by a job on the site's own repo — which is a nice dogfood of both features and not a launch dependency.
...
| D8 | `technical` is built from the contract — parts + settings + variations — with no forked layout from `base` | **green, 13 Sep 2026** — U6b (docs/11 §7b). `themes/technical` declares neither `layouts:` nor `primitives:`; `snypd://theme/coverage`, read out of a `bun build --compile` binary running where there is no `themes/` directory at all, reports all 13 primitives `inherited` `via base`, all five layouts, and parts `shell`/`footer`/`entries` inherited against `header`/`toc` own. Two variations, eight settings, 40 tokens, and no webfont — `tech.font.kb` **0 / 0** |
...
| D12 | snypd.rocks has `/themes`, `/plugins`, `/bench` (S22), a nav, and **one post per gate** — the U-series (after U6b), the P-series (after P4), and the launch itself (L2) — build-in-public is the launch content (amended 6 Sep 2026, decision 97: three good posts over fourteen thin ones) | **the pages answer 200 — 15 Sep 2026 (S22 · L1)**, each written through the MCP and published under the `page` type's own policy; the header menu names them (`site › set_nav`), and the live site renders that menu — and the masthead's tagline, decision 146 — once the host builds with a release ≥ 0.1.4 (E3, the same wait D7 has). The three posts: two of three in the feed; the launch post is L2's |
...
| 10 | ~~**U6b**~~ | **done 13 Sep 2026** — `technical` from the contract (no layouts, no primitives, no font: 13/13 and 5/5 inherited from `base`, two parts its own), the `build-theme` prompt (§5.3), and the design pass at 390 and 1280 as its own `tech.*` browser lane. `base` gains a fifth part — `toc`, an empty contents slot in the post layout — and `Page.headings` the tree to fill it from; both cost a theme that ignores them nothing, and the byte-diff of `corpora/theme` under `editorial` is the proof: one file moved, `assets/theme.css`, and that was `@view-transition`. The log row is docs/11 §7b. No post: decision 97 | **D8 ✅** — `snypd://theme/coverage` out of a compiled binary in a directory with no `themes/`: 13 primitives `inherited via base`, five layouts, `parts: shell/footer/entries inherited, header/toc own` |
| 11 | **S19d · S20** | Phase 4 as scheduled — branch previews; the markdown-engine report and speed pass | docs/07 exits |
| 12 | ~~**S21**~~ | **done 15 Sep 2026** — all four plugins on in `corpora/kill`, one check per tier off the finished site, and three models at the keyboard (docs/11 decisions 169–170). The contract held: no model was told a plugin existed, and every finished site carried the beacon, the linked term, the `release` type and the key file. What the plugins *cost* was the finding — `tokens.learn` 6,529 on the site the test leaves — and the resource pays it now (decision 169) | D1 ✅ × 3 · D4 ✅ |
| 13 | ~~**S22 · L1**~~ | **done 15 Sep 2026** — docs/11 §7b: `/bench`, `/themes`, `/plugins` live, written through the MCP; `snypd bench gallery` is the screenshots script and a CI lane; `snypd://themes` is the shelf's read | **D12 ✅ (the pages), E9 ✅** — the nav reaches them when the host's pin moves (E3) |
