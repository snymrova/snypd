...
## 8. R2 outcome (18 Sep 2026)

Ferrule declares the registry of §2.1 — `work` extends `post` (`client` required, `year`, `service` as a list of refs, `industry` as one, `mcp.write: draft`), `service` hierarchical with *product / space / identity* as parents and the five old tags as their children (`parent:`), `industry` flat with four terms — and its content moved as §2.2 said: six cases in `content/work/` with a client each (*bloco* back to `draft`), three notes in `content/posts/` on three tags, the studio page at `/studio/` with `/studio/credits/` under it (`parent: studio`, and the generator writes it there), the masthead *Work · Notes · Studio*, eight redirects in `site.redirects` (the six cases, `/about`, `/credits`). Every self-link in the bodies and every `source=` on a stat moved with the pages.

Exit: `snypd lint` 0 errors, 2 warnings — rule 11 on `healthcare` and `publishing`, each one case's industry, which is what a six-case portfolio's industry facet looks like and not a mistake to fix with a fifth term; `snypd build` 30 routes (was 22): `/work/`, `/posts/`, five cases, three notes, nine service and industry terms, `/studio/credits/`, and the build line says *work renders through `post` — studio declares no `work` layout* until R3; `_redirects` carries the eight; `/service/product/` lists the three product cases; the front page's band is *Work* over five cards; `llms.txt` heads the type *Work*, as the menu does (the label follows the archive's title, R1). Page suite over nine routes at 1280 and 390: js 0, font 32.74 KB, **axe 0 across 18 pairs** and 0 scrolled, cls 0, `media.kb` 819.74 (the reel). The card grid on `/work/` shows no service under the date — the studio's card read `category`, which a `work` no longer has; R3 hands a card its terms.

## 9. R3 outcome (18 Sep 2026)

Built as §2.3 said, audited in docs/21: `layouts/work.tsx` (three bands, the facts strip, the close band with cta and next case), `layouts/work-index.tsx` (the lede *5 cases, 2025–2026.*), cards with a term (decision 198), the build line no longer says *work renders through post*. One departure: the author is not in the strip but under the close band (*Led by …*), so the strip is the four facts §2.3 named and nothing else. Exit numbers in docs/21 §2. R4 is what remains.

## 10. R4 outcome (18 Sep 2026)

Built as §2.4 said, in `packages/bench/agent/registry.ts` and recorded in docs/22: the twelve steps as data with the sentence each should see, a scripted route (18 calls with the harness's lint, gated exactly in `registry.test.ts`), a `claude:<model>` driver whose harness presses the review page's button when the model asks for the preview, and a judge that scores the transcript — the one lane in `bench/` that does, because the claim is about what the tools say. Four places on the surface had to change before they said it: `explain_config` names the inherited value a type's key overrides, a write prints its diagnostics, the publish commit carries who approved it (and `snypd://history` shows it), `site › build` names its lists and its fallbacks. Four departures from the table, kept and argued in docs/22 §2: the *inherited* line is `snypd://config`'s, the approval rides on the publish commit, the query asks for published, rule 10 stays quiet on a URL that never existed. The README section is written as launch copy (docs/22 §4) for `r0-readme-ground`; the stills and the film's second clip wait for the site's move to the studio look, as the hero film's reshoot does.
