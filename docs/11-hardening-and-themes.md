...
~~Session 1 is **H0**~~ · ~~session 2 is **U6a**~~ — both landed 13 Sep (§7b). H0 paid for U6a twice over:
`cssValue` already had `oklch`, `color-mix` and the relative colour syntax on its allow list, so every
value in the three variations was checked by machinery that was written the day before and needed no
change. Session 3 is **B1**, the font pass, and it needs nothing from anyone: `font.heading` exists now,
which is the token a webfont attaches to.
