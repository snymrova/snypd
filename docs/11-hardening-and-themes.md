...
Session 4 is **U6b**: `technical` from the contract, the `build-theme` prompt, and the design pass at 390
and 1280. It starts better off than B1 did. A second theme is now a theme that can ship its own face
without new machinery — `font:` is in the contract, `theme check` (X1, session 5) inherits it, and the
one thing B1 could not buy at 40 KB is the thing `technical` should probably not want anyway: a
monospace-led theme's emphasis is weight and colour, not a cursive.
