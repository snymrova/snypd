...
settings:
  ...
  - { id: dateFormat,  type: select,    label: "Date format", group: Posts,    default: iso, options: [iso, long, short] }   # `relative` was here until U3; see below
...
**Three things U3 settled that this section had left to the implementation** (6–12 Sep 2026, decisions
113–116):

1. **`settings:` is the one `theme.yaml` key that does not merge into the config.** Every other root key
   of a `theme.yaml` becomes `theme.*` — that is how `tokens:` and `parts:` reach the merged config — and
   a *list* of declarations merging onto `theme.settings` would sit exactly where the *map* of values has
   to be. So the loader reads it and drops it, the way the plugin manifest's `plugin:` block is read and
   never merged (docs/10 §4.1). `theme.settings` is the site's answers and nothing else, which is also
   what makes its provenance readable: every line in it came from `snypd.yaml`.
2. **A value is checked against the declaration at load.** A setting has a declared type, which is the one
   thing a token does not, so `theme.settings.showDates: "yes"` is an error and not a truthy string the
   renderer discovers at render. An id the theme does not declare is a *warning*, because a theme switch
   leaves values behind exactly as it leaves token overrides behind and neither should stop a build.
3. **There is no `relative` date format.** A statically built page cannot carry one honestly: "2 days ago"
   is in the HTML, it is wrong on day three, and the only ways to keep it true are client JS (this product
   ships none) or a daily rebuild (which an incremental build would correctly skip). `short` takes its
   place in the option list, and `formatDate` writes English month names from the string's own digits
   rather than through `toLocaleDateString`, whose answer depends on the ICU data of the machine that ran
   the build.
...
- A `ref` that resolves to nothing is **lint rule 5's** existing "dead internal link" with a new source, not
  a new rule — the vocabulary of failures stays small. (Written as "rule 7" until U2 landed; 7 is
  `stale-updated`.) A file for a location the theme does not declare is rule 12, `nav-location`.
- **What "survives a slug change" means here (U2, found building it):** the slug *is* the filename
  (write.ts), so `type/slug` names the file. It follows a url-pattern change and a theme switch; a
  renamed file makes it a dead ref that lint names. The route form — `ref: /about` — follows
  `site.redirects` and the index's move log, exactly as a body link does under rule 10, so the rename plus
  the redirect rule 10 asks for is what keeps a menu whole. `site` › set_nav refuses a dead ref outright.
...
| T2 | The settings surface is free until read | **green, 12 Sep 2026 (U3)** — `tokens.learn` 4,564 on `base`, byte-identical, and 4,708 on `editorial` (+18: one line in `snypd://theme` naming the surface, so an agent can find it), both under 6,000; `snypd://theme/settings` is a resource and is listed only by a theme that declares some. `tokens.tools` **2,230, byte-identical** with `set_settings` added — the `theme` tool is catalogue-only | `snypd bench agent` |
...
| T6 | Nav is real | header and footer menus render on `base` and `editorial`, from files, with a dead `ref` caught by lint rule 5 | `render.test.ts` + `corpora/theme` — **green, U2** |
...
| ~~**U2**~~ | **done 6 Sep 2026, PR #11** — `locations:` honoured; `content/nav/<location>.yaml` loaded and resolved; `ref` → route through the content list, redirects and the move log; dead `ref` on lint rule 5; `header`/`footer` parts render a real menu on both themes; `snypd://nav` resource; `site › set_nav`; snypd.rocks got both menus the same day (live once 0.1.3 ships — the host builds with the published CLI) | T6 green ✅ · a menu follows a redirect and a url-pattern change ✅ (§4.3 says what a rename does) · both themes ship a header a real blog can use ✅ |
...
|---|---|---|---|---|
| U1 | 2026-09-06 | #10 | No clocks from this box (memory: not comparable); CI's `bench.compare` is on the PR. Token rows against the record (`0.1.2`, 5 Sep): `tokens.tools` **2230 → 2230**, byte-identical — nothing was added to `tools/list`; `tokens.learn` 4494 → 4508 (+14: the coverage resource's description now names the four parts); `tokens.page.html` 1381 → 1510 report-only, the social tags — `tokens.page.md` 510 unchanged, and the twin carries none of it (§2.3 item 7 held); `assets/` still one file, 0 KB JS. tests 324 → 326, 0 fail; typecheck clean | **The shell is a part, and `editorial` changes the header with one file.** `parts:` in `theme.yaml`, resolved by the same `declarer` walk primitives use — one function with a map selector, not a second copy — so `{ fallback }`, nearest-declarer and `own \| inherited \| fallback \| missing` all hold for parts on day one. `base` moved `shell` and `entries` from `layouts/` to `parts/` and split `header` and `footer` out of the shell; the five layouts take `part(ctx, "shell")` and `part(ctx, "entries")`, which throws with the theme and the part named when a chain has neither. `editorial` declares `parts: { header: ./parts/header.tsx }` — a masthead with the site description as a tagline — and nothing else, and coverage reports `header: own`, the other three `inherited via base`. That is the U1 exit, and §2.3 items 1, 3 and 6 close with it. **`theme.yaml` is validated (T7, decision 73):** strict Zod in `@snypd/core`, run over every link in the chain inside `loadConfig` with `file:line` from the parse the chain walk already had; `layout:` for `layouts:` is an error, and the four keys docs/04 documents and nothing reads (`locations`, `variants`, `patterns`, `client`) are a *warning* naming the session that builds each — §2.2 said out loud, in the diagnostic, not the doc. **Social metadata is core** (docs/10 §5.1, decision 90, pulled into U1 because it is the same shell): `og:site_name\|type\|title\|description\|url\|image(+alt, width, height from `ctx.media`)`, `article:published_time\|modified_time`, `twitter:card` — from `cover.image` or the new `site.image`, `summary_large_image` only when there is one. A dated item is an `article`, everything else a `website`. **Cost, stated:** ~130 tokens of `<head>` on every HTML page, which is the price of a share card and lands on a report-only row; the `.md` twin, which is the gated row, is untouched. Also: `snypd://theme/coverage` gains `parts:`; `theme › scaffold`'s comment says how to override one; the S13 reload test now edits `parts/footer.tsx`, since no layout imports the shell any more. U2 next: the header both themes now own is where the menu goes |
| U2 | 2026-09-06 | #11 | No clocks from this box; CI's `bench.compare` is on the PR. Token rows against `0.1.2` via `bench --quick` here: `tokens.tools` 2230 → **2230**, byte-identical — `set_nav` and its two parameters sit on the `site` tool, which is catalogue-only and never in `tools/list` (decision 38 doing its job); `tokens.learn` 4508 → 4522 (+14: the `snypd://nav` row in `resources/list`); `tokens.page.html` report-only, +~150 on `corpora/theme` for two menus; `tokens.page.md` unchanged — the twin carries no menu; JS still 0 KB. tests 326 → 332 (331 pass, 1 skipped as before), 0 fail; typecheck clean | **A menu is a file, and both shipped themes render two of them.** `theme.yaml › locations` joins the Zod schema (the first of the four documented-but-unbuilt keys to become real); `base` declares `[header, footer]`, `editorial` inherits them because arrays *append* up the chain (merge.ts) — §4.3's guess that a child restates them was wrong, and the fixture now says so. `loadNav` reads `content/nav/<location>.yaml` as a list of `{ label, ref \| url, rel? }`, strict; `routeLookup` is the index, every item by `type/path`, every term page any frontmatter names, and the aliases — `site.redirects` plus the index's move log — so `ref: /about` follows a rename the way rule 10 expects a body link to. The resolved menus are hashed into every route key: a menu edit, or a slug change that moves an item, re-renders every page, and a no-op stays a no-op (T6 asserts both). **What "survives a slug change" turned out to mean** is in §4.3: the slug is the filename, so `type/slug` follows a url-pattern change and a renamed file is a dead ref; the route form follows the redirect. Lint: rule 5 on the nav file with the item's line; a file for an undeclared location is rule 12, a warning; `lintSite` carries both, so doctor and `content` › lint see a broken menu unasked. `site` › set_nav is the one write path — refuses an undeclared location, a malformed item and a dead ref before touching disk, writes one flow map per line, no-ops on the same menu — and commits + lands like every site write. `snypd://nav` is the read: locations, every file, every `ref` with what it resolved to. `menu(ctx, location, route)` in `@snypd/render` hands a part its items with `current` set; both themes emit `<nav aria-label>` + `<ul>` and `aria-current="page"`, and `editorial`'s masthead puts the menu beside the brand, wrapping under it on a phone. Doctor gained one row (`menus: header 5 items, footer 4 items`, or a ⚠ naming set_nav on a site with none). **snypd.rocks** got both menus the same day, through the tool, and was pushed — but the public page will not show them until 0.1.3 is on npm: the host's build command pins `@snypd/cli@0.1.2` (wrangler.toml), so the site is built by the released binary, not this branch. Found by curling the live site after the push; docs/10 §7.2 carries it as the open half of the exit. Also: the seed rule-5 route set is now the same lookup nav uses, so it knows term pages — a body link to `/category/engineering` stopped being a false dead link. Post per session: no — decision 97 (docs/10) makes it one per gate |
| U3 | 2026-09-12 | (this branch) | No clocks from this box (memory: not comparable; load average ~18 while the suite ran). CI's `bench.compare` is on the PR. Token rows via `bench --quick` here: `tokens.tools` 2230 → **2230**, byte-identical — `set_settings` and its `settings` parameter sit on the `theme` tool, which is catalogue-only (decision 38 again); `tokens.learn` 4564 → **4564** on `base`, byte-identical, because `base` declares no settings and the line is only printed by a theme that does; `tokens.learn.editorial` 4690 → **4708** (+18: that line); `tokens.tools.full` 3890 → 4020 report-only; `tokens.page.md` 510 and `.html` 1510 unchanged — a setting costs what the part that reads it emits, and the corpus sets none; JS still 0 KB. tests 359 → 368 (367 pass, 1 todo), 0 fail; typecheck clean | **A theme says what a site may choose, and the site's answer is checked before it is rendered.** `settings:` in `theme.yaml` is a list of `{ id, type, label, group?, default?, info?, options?, min?, max? }`, strict Zod with file:line, twelve types (`text textarea richtext url number boolean select color size font image link_list`), appended parent-first up the chain with a child's `id` replacing its parent's entry where it stands. `ctx.settings` is the renderer's reading — defaults applied, unanswered ids left out — with `settingText` / `settingFlag` / `settingLinks` as the cast, and the resolved map is in the route key, so changing a setting re-renders and changing it back is the bytes it was (asserted). `snypd://theme/settings` is the agent's read and is listed **only by a theme that declares some**; `theme` › set_settings is the write, refusing the whole patch before it writes any of it; doctor prints one row, plus a warning for values a theme switch stranded; `snypd://theme` gains one line so the surface is findable from the read an agent already makes. `editorial` declares the six §4.2 named — and four of them (`showDates`, `dateFormat`, `footerNote`, `social`) are read by parts **`base`** owns, which is decision 116 and the reason a child gets them without forking a part. **Five things U3 found:** (1) `settings:` cannot merge into the config like every other `theme.yaml` key — a declaration *list* would land exactly where the *value map* has to be, so it is read and dropped, the plugin manifest's rule one file over (decision 113); (2) a setting has a declared type and a token does not, so a value can be checked at load — `showDates: "yes"` is an error, `setConfig` rolls back, and a hand edit is refused instead of read as truthy at render (decision 114); (3) `relative` cannot be an honest date format in a statically built page, and `short` takes its place (decision 115); (4) `toLocaleDateString` would make the same content two different pages on two different boxes, so `formatDate` writes English month names from the string's own digits — S18d′'s rule, arriving by another door; (5) an entry that fails the strict parse is refused *whole* rather than stripped of the bad key, because a declaration with a key the loader does not understand is one nobody can rely on. **Checked through the compiled artefact and not only the source lane** (the S18a rule, P4's precedent): `bun build --compile --splitting`, then `serve` over stdio on a site using the bundled `editorial` — the resource is listed and read, the summary line and the doctor row are there, and a wrong-typed value is refused. A bundled theme has no directory, and settings are read from `theme.yaml` through the `themefs` seam, which is exactly the shape of S18d's stranded-token bug. No post: decision 97 |
...
75. **A menu is a file.** `content/nav/<location>.yaml`, resolved by `ref` through the index, linted by rule
    5 (not 7 — corrected in U2). No `nav` content type in v0.1.5 — a list needs no status machine.
