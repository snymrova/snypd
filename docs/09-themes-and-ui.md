...
| ~~**U2**~~ | **done 6 Sep 2026, PR #11** — `locations:` honoured; `content/nav/<location>.yaml` loaded and resolved; `ref` → route through the content list, redirects and the move log; dead `ref` on lint rule 5; `header`/`footer` parts render a real menu on both themes; `snypd://nav` resource; `site › set_nav`; snypd.rocks got both menus the same day | T6 green ✅ · a menu follows a redirect and a url-pattern change ✅ (§4.3 says what a rename does) · both themes ship a header a real blog can use ✅ |
