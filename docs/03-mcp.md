...
**content.** *(S11 ships `create` `update` `query` `lint` `set_status` `publish` `trash` `restore`; the rest follow the schedule.)* `create(type, slug?, frontmatter, body)` · `update(type, slug, patch|body)` · `lint` · `suggest_blocks(markdown)` — upgrades plain prose into primitives · `render_preview(type, slug|markdown, theme?, viewport?)` → screenshot + URL · `query({type, taxonomy, fields, status, sort, limit})` · `set_status` · `publish` (elicits approval when policy is `draft`) · `schedule(publishAt)` · `trash` · `restore` · `translate(slug, locale)` · `explain(slug)`
...
**How S11 enforces that.** A write goes to `snypd/draft-<type>-<slug>`, cut from whatever branch was checked out, and only the paths that write touched are staged — `main` never carries a half-written post, and an unrelated edit in the tree stops the branch switch rather than riding along. `content.publish` on a `draft`-policy type is refused until a human approves on `/_snypd/review/{type}/{slug}`, and the approval names the content hash it covers: edit after approving and the publish is refused again, with the reason. Approvals live in `.snypd/approvals.json` — the preview server writes it, the MCP server spends it, and losing the file can only ever *refuse* a publish. Publishing merges the draft branch into its recorded base and deletes it. A site that is not its own git top level (a corpus inside another repo, say) gets the file writes and no commits at all: a benchmark must never be able to commit.
