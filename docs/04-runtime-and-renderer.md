...
- The comparison is a row in every bench run, not a verb: `build.cold.<n>.parse.bun` (S20, docs/11 decision 168) runs `Bun.markdown.html` over the sources the build just parsed and reports the ratio beside the parse row — the ceiling a directive-capable parser could reach, watched rather than remembered. **S5 measurement:** micromark ≈ 0.4 MB/s on the corpus in Bun 1.4 and Node 22 alike (`Bun.markdown` ≈ 45 MB/s). **F1/S20 (Sep 2026):** the parser is 74–77 % of a cold build at every size and the build is at 14 % of its budget, so the parser stays; the switch is a v0.2 spike whose cost is the seam — a `:::` and frontmatter pre-split, a positioned tree, one output two engines agree on — and it opens when `build.cold.1000` in CI passes half its budget. Everything downstream is insulated by the hash-keyed mdast cache (`@snypd/core` `MdastCache`): lint, rebuilds and `snypd://lint/*` never re-parse an unchanged file.
...
- **SSR / preview (S11, `preview.ts`; owned by `snypd dev` since S18e).** The preview is the same incremental build, not a second renderer: `build({ drafts: true })` into `.snypd/preview` against its own index (`preview.sqlite`), so the production route cache is untouched and a preview never writes to `dist/`. `drafts` widens the filter from "public statuses" to "everything but trashed" and joins the route key, so the two builds can never hand each other an output. A watcher on `content/`, `snypd.yaml` and the theme dir sets a dirty flag; only a flagged request rebuilds, which is why TTFB is the static floor (**S11: 0.15 ms, budget 50**) and not a build time. The extra page is `/_snypd/review/{type}/{slug}` — the draft's frontmatter, its diff against the branch it was cut from, and the Approve button that records the approval `content.publish` requires (docs/03). It is rendered by the *theme's* `page` layout, so it inherits whatever the site looks like and no admin UI exists.

  **S18e — who owns it, and what may differ from `dist/`.** The preview stopped belonging to whichever MCP session asked for it first and started belonging to the person: `snypd dev` binds it, records `{url, port, pid, root}` in `.snypd/dev.json`, and `content.render_preview` finds that server over HTTP (`/_snypd/alive`) instead of binding a second one — which is what closes the collision both callers used to have on port 4321 (`07` decision 51, docs/08 §12.3). Without a `dev` running, the session-scoped behaviour is exactly as it was. Two things are preview-only and both live **in the response, never in the bytes**: the reload and a fixed link back to the Desk. **S18k made the reload a change stream.** `/_snypd/live` is server-sent events, the page carries a ~200-byte `EventSource` listener, and it reloads when the watcher says a rebuild is owed — debounced 80 ms, so one save is one reload — rather than every N seconds whether or not anything moved; `--reload=N` restores the S18e `Refresh` header for anyone who wants the poll, and `--no-reload` turns both off. That listener is the `<script>` S18e spent decision 51 refusing, and the amendment recorded there is why the refusal did not survive: the rule's reason is that the preview serves what publishes, and what proves that is the disk claim below, which a response-path injection cannot reach. The Desk takes neither — it keeps its own meta refresh, because its status card moves on harness activity no file watcher can see. `render.test.ts` asserts that every item page in `.snypd/preview` is byte-identical to the one `dist/` publishes, which is the whole claim of "what you see is what publishes"; index and feed pages differ by exactly the drafts, which is what the preview is for. Hosted SSR (scheduled posts, per-request `/ask`) uses the same fetch handler; a Workers adapter is a plugin.
- **Themes are TSX.** `theme.yaml` maps each primitive to a `.tsx` file exporting `(props, ctx) => JSX`. Rendered to strings. Theme authors (mostly agents) get a strict, tiny contract: props from the spec, `ctx.tokens`, `ctx.slots`, nothing else. **X1:** the check is `snypd check theme <name>` — not a type-check against the spec, which `tsc` already does, but sixteen named rules over the whole of the contract below plus the two things a type never sees: whether a variation invented a token, and whether the palette is readable (docs/11 §7b, decisions 139–144). `snypd new theme <name>` writes the starting point it judges.
  **S18f — the two states a new site is in, and where the Desk's facts come from.** A site with zero items serves an index the dev server *renders* — the theme running five primitives through `renderDoc` (lifted out of `build()` for this second caller), behind a strip saying only you can see it. It exists in no file, `build()` never emits it, and it disappears on the first real post, so there is nothing a new site has to delete and nothing that can leak into `dist/` (`07` decision 52). The Desk above it grows a first-run checklist, and every fact on it comes from `onboardingFacts` in `@snypd/core` — the same function `site` › doctor renders as sentences, which is decision 64's rule made structural rather than promised. The sixth of those facts is the heartbeat, and it stopped being process-local: `.snypd/activity.json` is written by the MCP server on a leaf import and read by whatever renders the page, which since S18e is normally a different process (`07` decision 70).
...
Every key below is checked by rule: `snypd check theme <name>` reads the whole chain — the parts, the primitives, the token declarations, the variations, the font against its own budget, the stylesheet's refusal to `@import` — and prints one line per rule with the evidence, out of a source tree or a compiled binary alike. It is what the shelf runs (docs/10 §6).
...
settings:                                # U3 (docs/09 §4.2): what a site may choose without writing CSS. The values live in
  - { id: showDates, type: boolean, label: "Show dates", group: Posts, default: true }        # snypd.yaml › theme.settings and this key
  - { id: social, type: link_list, label: "Social links", group: Footer }                     # alone does not merge into the config
variations:                              # U6a (docs/10 §5.2): complete named looks, chosen with one word — snypd.yaml › theme.variation
  paper: { description: "Warm cream, oxblood accent." }                        # no tokens: the theme's own, named so a site can switch back
  ink:   { description: "Dark only.", tokens: { color.bg: "oklch(.17 .012 250)" } }   # values only — a variation retunes, it cannot declare
font:                                    # B1 (decision 118): the one webfont a theme may ship — self-hosted, subsetted, variable, WOFF2
  family: Source Serif 4                 # the CSS family name, and the one a font.* token has to say
  file: ./fonts/source-serif-4-latin.woff2   # theme-relative; emitted as assets/fonts/<basename> and preloaded in the shell
  weight: 400 700                        # a range, because it is variable: body, headings and `strong` out of one file
  kb: 31                                 # what it claims to cost. `page.font.kb` gates on this, ≤ 40; a file over its own claim is refused at load
  fallback:                              # the metric-matched stand-in, so `font-display: swap` shifts nothing (page.cls stays 0)
    local: Georgia                       # scripts/vendor-font.sh computes the four overrides from both fonts and prints this block
    size-adjust: 106.2%
    ascent-override: 97.6%
    descent-override: 31.6%
    line-gap-override: 0%
variants: { callout: [soft, loud], cover: [text-only, image-left, image-full] }   # NOT BUILT — docs/09 §4.5, deferred; warns since U1 (and not `variations`, above, which is built)
primitives:
...
**Settings since U3 (12 Sep 2026, docs/09 §4.2, decisions 113–116).** Twelve types (`text textarea richtext url number boolean select color size font image link_list`), declared as a list so the order is the theme's own, appended parent-first up the chain with a child's `id` replacing its parent's entry in place. Three things follow from a setting being *declared*: `ctx.settings` is typed and defaulted for the renderer (`settingText` / `settingFlag` / `settingLinks`), `snypd://theme/settings` and `theme` › set_settings are the agent's read and write, and a value of the wrong shape is an error at config load rather than something the renderer discovers — which is the one check `set_tokens` cannot make, because a token has no declared type. `settings:` is the only key in this file that does **not** merge into `theme.*`: the declarations are a list and `theme.settings` is the site's value map, so the loader reads it and drops it, exactly as it reads and drops a plugin's `plugin:` block (docs/10 §4.1).

**Variations since U6a (13 Sep 2026, docs/10 §5.2, decision 91).** A variation is a named set of token *values* over the theme's own defaults, and `theme.variation: ink` in `snypd.yaml` is the whole of what a site writes to take one. Precedence is **theme defaults ← variation ← the site's `theme.tokens`**, so an override a site made by hand still wins over the look it then chooses; the variation is merged as its own layer, which is what lets `snypd://config` attribute each token to the line of `theme.yaml` that wrote it rather than to the theme in general. Values, not declarations: a variation may retune a token but not add one, change its `kind`, or move `customisable` — that line is what separates a variation from a child theme, and it is what makes "switching changes exactly the tokens it names" a property a test can assert. Like `settings:`, `variations:` does not merge into `theme.*`: it is a declaration, and a site that could write one in `snypd.yaml` would be writing a look nothing could apply, since the site layer merges after the chosen variation has already landed. A name the theme does not ship is a warning and its own tokens render, exactly as a stranded token override is; `theme` › set takes `variation` beside `name`, `snypd://theme` carries the names and `snypd://theme/variations` what each one is.

**One webfont since B1 (13 Sep 2026, decision 118).** A theme may ship exactly one, and `font:` is where it says so. Self-hosted (cross-origin font caching died in 2021, so the shared-cache argument is gone), subsetted, variable, WOFF2, `font-display: swap`, preloaded from the shell — and with a **metric-matched fallback face** whose `size-adjust`, `ascent-override`, `descent-override` and `line-gap-override` are computed from both fonts, so the face a browser paints before the webfont arrives occupies exactly the space it will. That last part is what lets a webfont exist here at all: `page.cls` was 0 before B1 and is 0 after it, and a swap that moved the page would have been the budget bought with the wrong currency. The generated fallback family is the theme's name plus ` fallback`, and it goes between the webfont and the rest of the stack in `font.body`.

The budget is **declared, not discovered**. `kb:` is the theme's claim about its own file, at most 40 (decision 118); `loadTheme` refuses a claim above the ceiling and a file above its claim, and the `page.font.kb` lane gates the bytes on the wire against the same number — so a face that grew has to be re-subsetted or re-declared in the commit that grew it, and a lane that measured the file against itself could never fail. **Nearest declarer wins**, as it does for primitives, parts and tokens: a child inherits its parent's face until it names its own, and two faces never stack up by inheritance. Like `settings:` and `variations:`, `font:` does not merge into `theme.*` — it is a declaration about a file in the theme's directory, there is nothing in it for a site to answer, and `theme.font` written in `snypd.yaml` is a warning saying so (decision 131). The `@font-face` pair is emitted *above* the layer statement and inside no layer, because a `@font-face` takes part in no cascade. `scripts/vendor-font.sh` is how the file and the four overrides are made, and re-running it is how they are checked.

**Validated since U1 (6 Sep 2026, docs/09 decision 73).** `theme.yaml` is strict Zod: a mistyped key is an error naming file and line, and the three keys above that are documented and not yet read (`variants`, `patterns`, `client` — `locations` became real in U2) are a *warning* that says which session builds them, rather than a key silently discarded. **Parts** (`shell`, `header`, `footer`, `entries`) resolve up the chain exactly as primitives do and appear in `snypd://theme/coverage`; a layout takes `part(ctx, "shell")` from `@snypd/render` instead of importing `./shell`, which is what lets a child theme replace the header with one file. The shell also emits social metadata — `og:*`, `article:*`, `twitter:card` — from frontmatter (`cover.image`, `cover.alt`, `date`, `updated`) and `site.image` as the fallback image (docs/10 §5.1, decision 90).
