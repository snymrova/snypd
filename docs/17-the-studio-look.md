...
> **Outcome, 17 Sep 2026 — built the same day, as S29 (docs/11 §7b, decisions 183–187).** Sunny's answer to §7 was *"lets work on the studio look"*, the same afternoon, so the four calls are taken as recommended and the slot moved: bundled (183), one autoplay per page (184, amending 181), `logo-wall` as primitive fourteen (185), and S29 before Gate D rather than after it (186) — the film's re-shoot waits on the site, and the site waits on this. **Shipped from §6:** item 1, the theme (`themes/studio/`: bands by `color-scheme` per `##` section over a `sections` seam the renderer now hands every layout, Bricolage at 32.6 KB, hairline cards, pill buttons, the sticky blurred masthead, cards for the entry list); §4.1–4.2 `cover.media` and `autoplay` with the four attributes the renderer writes, the still under reduced motion in `base`'s sheet, lint rule 15; §4.3 `logo-wall` with its detector and rewriter, lint rule 16, three posts in the suggest corpus (precision stays 1.0 over 53); the tier-B half — reveals on `translate` only, counters over a `--n` the renderer sets inline, the marquee, the masthead condensing under `scroll-state()`; `page.media.kb` and `gallery.*.media.kb`, report-only, and the gallery at seven looks. **Refused by the gate, as §5 said it would be:** the masthead that *flips* with the band under it — `mix-blend-mode: difference` is the CSS-only way, axe cannot see a blend, and it read white on the post's light page as five contrast failures (`gallery.studio.a11y.violations` 0 → 1), so the bar carries the hero's scheme instead. **Not shipped, with the reason:** the two carousels and the tabs (§3) — `pullquote × n` and sibling `steps` have no container element to scroll-snap; that is the pages-vocabulary question again, not a theme's. **Owed:** snypd.rocks switches to the look once the host builds with 0.1.5; the README on `r0-readme-ground` still says thirteen; Safari by hand.
