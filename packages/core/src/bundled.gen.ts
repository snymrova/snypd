...
export function generate(): string {
  const imports: string[] = [], entries: string[] = [], pluginEntries: string[] = [];
  const pack = (kind: "themes" | "plugins", name: string, into: string[]) => {
    const dir = join(kind === "themes" ? THEMES : PLUGINS, name);
...
if (import.meta.main) {
  ...
  console.log(`wrote ${out} (themes ${BUNDLED_NAMES.join(", ")}; plugins ${BUNDLED_PLUGIN_NAMES.join(", ")})`);
}
