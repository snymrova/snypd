...
beforeAll(async () => {
  ...
  ctx = { site: { name: "P", url: "https://p.example" }, tokens: resolveTokens({}), theme: { name: theme.name }, assets: { feed: "/feed.xml", llms: "/llms.txt", api: "/api/site.json" }, media: { "/media/one.png": { width: 64, height: 40 } }, config: cfg.config, parts: theme.parts, nav: {}, hooks: EMPTY_HOOKS, settings: {}, preview: false, pieces: {} };
});
